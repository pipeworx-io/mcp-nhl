...
async function pwFetch(url: string | URL, init?: RequestInit): Promise<Response> {
  return fetchWithTimeout(url, init ?? {}, 'NHL');
}
...
async function getStandings() {
  const res = await pwFetch(`${BASE_URL}/standings/now`);
  if (!res.ok) throw await httpError(res, 'NHL API error');
...
async function getScores() {
  const res = await pwFetch(`${BASE_URL}/score/now`);
  if (!res.ok) throw await httpError(res, 'NHL API error');
...
async function getSchedule() {
  const res = await pwFetch(`${BASE_URL}/schedule/now`);
  if (!res.ok) throw await httpError(res, 'NHL API error');
...
async function getPlayer(playerId: number) {
  const res = await pwFetch(`${BASE_URL}/player/${playerId}/landing`);
  if (!res.ok) throw await httpError(res, 'NHL API error');
